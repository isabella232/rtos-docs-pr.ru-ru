...
Разработка внедренных решений обычно выполняется на компьютерах Windows или Linux (Unix). После компиляции приложения, его связывания и создания исполняемого файла на узле он скачивается на целевое оборудование для выполнения.
...
| NX_ENABLE_SOURCE_ADDRESS_CHECK | Определено. Включает проверку адреса источника во входящем пакете. Этот параметр по умолчанию отключен. |
| NX_LITTLE_ENDIAN | Определено. Выполняет необходимую перестановку байтов в средах с прямым порядком байтов, чтобы гарантировать, что заголовки протокола находятся в правильном формате с прямым порядком байтов. Обратите внимание, что значение по умолчанию обычно устанавливается в ***nx_port.h***. |
...
| NX_ARP_MAX_QUEUE_DEPTH | Указывает максимальное количество пакетов, которые могут быть поставлены в очередь во время ожидания ответа ARP. Значение по умолчанию равно 4 и определено в ***nx_api.h***. |
...
| NX_DISABLE_ARP_AUTO_ENTRY | Определено. Отключает ввод данных запроса ARP в кэш ARP. |
| NX_DISABLE_ARP_INFO | Определено. Отключает сбор данных ARP. |
| NX_ENABLE_ARP_MAC_CHANGE_NOTIFICATION | Определено, разрешает ARP вызывать функцию уведомления обратного вызова при обнаружении изменения MAC-адреса. |

### <a name="icmp-configuration-options"></a>Параметры конфигурации ICMP  

| Параметр  | Описание  |
|---|---|
| NX_DISABLE_ICMP_INFO | Определено. Отключает сбор данных ICMP. |
...
| Параметр  | Описание  |
|---|---| 
| NX_DISABLE_IGMP_INFO | Определено. Отключает сбор данных IGMP. |
...
| NX_MAX_MULTICAST_GROUPS | Указывает максимальное число групп многоадресной рассылки, которые могут быть объединены. Значение по умолчанию равно 7 и определено в **nx_api.h** _. Приложение может переопределить значение по умолчанию путем определения значения до включения _ *_nx_api.h_**. |
...
| NX_DISABLE_IP_INFO | Определено. Отключает сбор данных IP. |
...
| Параметр  | Описание  |
|---|---|
| NX_DISABLE_RARP_INFO | Определено. Отключает сбор данных RARP. |

### <a name="tcp-configuration-options"></a>Параметры конфигурации TCP

| Параметр  | Описание  |
|---|---|
| NX_DISABLE_RESET_DISCONNECT | Определено. Отключает обработку сброса во время отключения, если предоставленное значение времени ожидания указано как **NX_NO_WAIT**. |
| NX_DISABLE_TCP_INFO | Определено. Отключает сбор данных TCP. |
...
| NX_ENABLE_TCP_KEEPALIVE | Определено. Включает дополнительный таймер проверки активности TCP (keepalive). Параметры по умолчанию не включены. |
| NX_ENABLE_TCP_MSS_CHECKING | Определено. Включает проверку минимального значения MSS у узла сети перед принятием TCP-подключения. Чтобы использовать эту функцию, необходимо определить символ **NX_ENABLE_TCP_MSS_MINIMUM**. По умолчанию этот параметр отключен. |
...
| NX_TCP_KEEPALIVE_INITIAL | Указывает число секунд бездействия до активации таймера проверки активности (keepalive). Значение по умолчанию равно 7200, что соответствует 2 часам, и определяется в ***nx_tcp.h** _. Приложение может переопределить значение по умолчанию, определив его до включения _*_nx_api.h_**. |
...
| Параметр  | Описание  |
|---|---|
| NX_DISABLE_UDP_INFO | Определено. Отключает сбор данных UDP. |
