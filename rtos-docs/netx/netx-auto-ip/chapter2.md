...
- **nx_auto_ip.h**: заголовочный файл AutoIP для NetX.
- **nx_auto_ip.c**: исходный файл C AutoIP для NetX.
- **demo_netx_auto_ip.c**: исходный файл C демонстрации работы AutoIP для NetX.
- **nx_auto_ip.pdf**: описание AutoIP для NetX в формате PDF.

## <a name="autoip-installation"></a>Установка AutoIP

Чтобы использовать AutoIP для NetX, следует скопировать упомянутый выше дистрибутив целиком в тот же каталог, где установлен NetX. Например, если NetX установлен в каталоге " *\threadx\arm7\green*", то файлы *nx_auto_ip.h*, *nx_auto_ip.c* и *demo_netx_auto_ip.c* необходимо скопировать в этот каталог.

## <a name="using-autoip"></a>Использование AutoIP

Использовать AutoIP для NetX просто. В коде приложения следует включить файл *nx_auto_ip.h* после *tx_api.h* и *nx_api.h*, чтобы использовать ThreadX и NetX. После включения файла *nx_auto_ip.h* код приложения сможет выполнять вызовы функций AutoIP, указанные далее в этом руководстве. В приложение также следует включить файл *nx_auto_ip.c* для процесса сборки. Эти файлы должны быть скомпилированы так же, как и другие файлы приложения, а их форма объекта должна быть связана с файлами приложения. Это все, что необходимо для использования AutoIP для NetX.

> [!NOTE]
> Так как AutoIP использует службы ARP NetX, необходимо включить ARP с помощью вызова *nx_arp_enable* перед использованием AutoIP.

## <a name="small-example-system"></a>Небольшой пример системы

Пример того, насколько просто использовать AutoIP NetX, приведен на рисунке 1.1 ниже. В этом примере заголовочный файл AutoIP *nx_auto_ip.h* подключается в строке 002. Затем создается экземпляр AutoIP NetX в "*tx_application_define*" в строке 090. Обратите внимание, что блок управления AutoIP NetX "auto_ip_0" был ранее определен как глобальная переменная в строке 015. После успешного создания AutoIP NetX запускается в строке 098. Обработка функции обратного вызова изменения IP-адреса начинается со строки 105, которая используется для обработки последующих конфликтов или возможного разрешения адресов DHCP.

> [!NOTE]
> В приведенном ниже примере предполагается, что основное устройство является устройством с одной сетевой адресацией. Для многосетевого устройства ведущее приложение может использовать службу AutoIP NetX *nx_auto_ip_interface_* , настроенную для указания дополнительного сетевого интерфейса для проверки IP-адреса. Дополнительные сведения о настройке многосетевых приложений см. в **Руководстве пользователя по NetX**. Обратите внимание, что ведущее приложение должно использовать API NetX *nx_status_ip_interface_check*, чтобы убедиться, что AutoIP получил IP-адрес.
...
Существует ряд параметров конфигурации для создания AutoIP NetX. Ниже приведен список всех параметров с описанием.

- **NX_DISABLE_ERROR_CHECKING**: определен, этот параметр удаляет базовую проверку ошибок AutoIP. Обычно он включается после завершения отладки приложения.
- **NX_AUTO_IP_PROBE_WAIT**: время ожидания в секундах перед отправкой первой пробы. По умолчанию значение равно 1.
- **NX_AUTO_IP_PROBE_NUM**: количество проб ARP для отправки. По умолчанию значение равно 3.
- **NX_AUTO_IP_PROBE_MIN**: минимальное время ожидания в секундах между отправкой проб. По умолчанию значение равно 1.
- **NX_AUTO_IP_PROBE_MAX**: максимальное время ожидания в секундах между отправкой проб. По умолчанию значение равно 2.
- **NX_AUTO_IP_MAX_CONFLICTS**: количество конфликтов AutoIP перед увеличением задержки обработки. По умолчанию значение равно 10.
- **NX_AUTO_IP_RATE_LIMIT_INTERVAL**: время в секундах, в течение которого будет продлен период ожидания при превышении общего числа конфликтов. По умолчанию значение равно 60.
- **NX_AUTO_IP_ANNOUNCE_WAIT**: время ожидания в секундах перед отправкой объявления. По умолчанию значение равно 2.
- **NX_AUTO_IP_ANNOUNCE_NUM**: количество отправляемых объявлений ARP. По умолчанию значение равно 2.
- **NX_AUTO_IP_ANNOUNCE_INTERVAL**: время ожидания в секундах между отправками объявлений. По умолчанию значение равно 2.
- **NX_AUTO_IP_DEFEND_INTERVAL**: время ожидания в секундах между объявлениями защиты. По умолчанию значение равно 10.