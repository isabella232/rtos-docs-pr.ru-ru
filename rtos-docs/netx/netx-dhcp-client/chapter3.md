...
Эта служба создает экземпляр DHCP для ранее созданного экземпляра IP. По умолчанию протокол DHCP включается для основного интерфейса. Введенное имя, хотя и не используется в реализации DHCP-клиента в NetX, должно соответствовать требованиям RFC 1035 к именам узлов. Общая длина не должна превышать 255 символов, а элементы, разделяемые точками, должны начинаться с буквы, заканчиваться буквой или цифрой и могут содержать только буквы, цифры и дефисы.
...
Эта служба включает протокол DHCP для указанного интерфейса. По умолчанию клиент DHCP включен для основного интерфейса. На этом этапе можно запустить DHCP либо на этом интерфейсе, вызвав *nx_dhcp_interface_start*, либо на всех интерфейсах с поддержкой DHCP, вызвав *nx_dhcp_start*.
