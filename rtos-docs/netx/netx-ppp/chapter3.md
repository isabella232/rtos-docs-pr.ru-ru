...
ms.locfileid: "116798764"
---
# <a name="chapter-3---description-of-azure-rtos-netx-point-to-point-protocol-ppp-services"></a>Глава 3. Описание служб протокола PPP для NetX в ОСРВ Azure

В этой главе приводится описание всех служб PPP для NetX в ОСРВ Azure, которые перечислены ниже в алфавитном порядке.
...
Эта служба создает экземпляр PPP для указанного экземпляра IP-адреса NetX.
...
Эта служба возвращает IP-адрес DNS-сервера, предоставленный одноранговым узлом. Если одноранговый узел не предоставил IP-адрес, возвращается значение 0.

### <a name="input-parameters"></a>Входные параметры

- **ppp_ptr**: указатель на блок управления PPP
- **dns_address_ptr**: назначение для IP-адреса DNS

### <a name="return-values"></a>Возвращаемые значения

- **NX_SUCCESS** (0x00) — адрес PPP успешно получен.
...
Эта служба настраивает IP-адреса локального и однорангового узлов для использования в протоколе IPCP (протокол управления протоколом IP). Приложение PPP должно вызывать эту службу в экземпляре PPP с допустимыми IP-адресами для самого себя и другого однорангового узла.  Если в экземпляре PPP нет зарегистрированных допустимых адресов, для определения его IP-адреса следует использовать одноранговый узел PPP.
...
Эта служба отправляет запрос проверки связи LCP и устанавливает флаг, означающий, что устройство PPP ожидает ответ проверки связи. Служба возвращает управление сразу после отправки запроса. Она не ждет никакого ответа. 
