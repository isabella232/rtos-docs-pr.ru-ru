...
Эта служба создает пул блоков памяти фиксированного размера. Указанная область памяти делится на максимально возможное количество блоков памяти фиксированного размера по формуле:
...
Назначение приоритетов списку приостановки пула блоков
...
Эта служба размещает поток с наибольшим приоритетом, приостановленный для выделения блока памяти в этом пуле, в начало списка приостановки. Все остальные потоки остаются в том же порядке FIFO, в котором они были приостановлены.
...
Получение сведений о производительности системы пулов байтов
...
Назначение приоритетов списку приостановки пула байтов
...
Эта служба размещает поток с наибольшим приоритетом, приостановленный для выделения памяти в этом пуле, в начало списка приостановки. Все остальные потоки остаются в том же порядке FIFO, в котором они были приостановлены.
...
Освобождение байтов обратно в пул памяти
...
Эта служба получает флаги событий из указанной группы флагов событий. Каждая группа флагов событий содержит 32 флага. Каждый флаг представлен одним битом. Эта служба может извлекать различные сочетания флагов событий, указанные входными параметрами.
...
Эта служба устанавливает или снимает флаги событий в группе флагов событий в зависимости от указанного параметра set_option. Все приостановленные потоки, для которых запрос флагов событий теперь удовлетворен, возобновляются.
...
Уведомлять приложение, когда заданы флаги событий
...
- **priority_inherit** — указывает, поддерживает ли этот мьютекс наследование приоритета. Если это значение равно TX_INHERIT, то наследование приоритета поддерживается. Если указано значение TX_NO_INHERIT, то мьютекс не поддерживает наследование приоритета.
...
Эта служба пытается получить монопольное владение указанным мьютексом. Если вызывающий поток уже владеет мьютексом, то внутренний счетчик увеличивается и возвращается состояние успешного выполнения.
...
Назначение приоритета в списке приостановки мьютекса
...
Эта служба размещает поток с наибольшим приоритетом, приостановленный для получения владения мьютексом, в начало списка приостановки. Все остальные потоки остаются в том же порядке FIFO, в котором они были приостановлены.
...
Эта служба уменьшает счетчик владения указанного мьютекса. Если счетчик владения равен нулю, мьютекс становится доступным.
...
Если очередь заполнена, сообщения всех приостановленных потоков будут удалены. После этого каждый приостановленный поток возобновляет работу с состоянием возврата, указывающим, что сообщение успешно отправлено. Если очередь пуста, эта служба не выполняет никаких действий.
...
Эта служба помещает поток с наибольшим приоритетом, приостановленный для получения (или размещения) сообщения в этой очереди, в начало списка приостановки.
...
Эта служба получает сообщение из указанной очереди сообщений. Полученное сообщение **копируется** из очереди в область памяти, указанную указателем назначения. Затем это сообщение удаляется из очереди.
...
Эта служба регистрирует функцию обратного вызова для уведомлений, которая вызывается при каждой отправке сообщения в указанную очередь. Обработка обратного вызова уведомления определяется приложением.
...
Эта служба создает семафор со счетчиком для синхронизации между потоками. Начальное значение счетчика семафора указывается в качестве входного параметра.
...
Удаление семафора со счетчиком
...
Получение экземпляра из семафора со счетчиком
...
Получение сведений о производительности семафора
...
Назначение приоритета в списке приостановки семафора
...
Эта служба размещает поток с наибольшим приоритетом, приостановленный для получения экземпляра семафора, в начало списка приостановки. Все остальные потоки остаются в том же порядке FIFO, в котором они были приостановлены.
...
Размещение экземпляра в семафоре со счетчиком
...
Уведомление приложения о выполнении операции размещения в семафоре
...
Эта служба регистрирует функцию обратного вызова для уведомлений, которая вызывается при каждой операции размещения в семафоре. Обработка обратного вызова уведомления определяется приложением.
...
Эта служба создает поток приложения, который запускает выполнение с указанной функции входа в задачу. Стек, приоритет, порог вытеснения и временной срез являются атрибутами, задаваемыми входными параметрами. Кроме того, также указывается начальное состояние выполнения потока.
...
- **preempt_threshold** — уровень наивысшего приоритета (от 0 до (TX_MAX_PRIORITIES-1)) при отключенном вытеснении. Только приоритеты выше этого уровня могут вытеснять этот поток. Это значение должно быть меньше или равно указанному приоритету. Значение, равное приоритету потока, отключает приоритет вытеснения.
- **time_slice** — количество тактов таймера, которое этот поток может выполнять до того, как другие готовые потоки с таким же приоритетом получают возможность запуска. Обратите внимание, что при использовании порогового значения вытеснения временной срез отключается. Допустимое значение временного среза находится в диапазоне от 1 до 0xFFFFFFFF (включительно). Значение **TX_NO_TIME_SLICE** (значение 0) отключает временной срез для этого потока.
...
- **auto_start** — указывает, запускается ли поток немедленно или помещается в приостановленное состояние. Допустимые значения: **TX_AUTO_START** (0x01) и **TX_DONT_START** (0x00). Если указано TX_DONT_START, приложение должно затем вызвать tx_thread_resume для выполнения потока.
...
Уведомление приложения при входе и выходе из потока
...
Эта служба регистрирует функцию обратного вызова для уведомлений, которая вызывается при каждом входе и выходе из потока. Обработка обратного вызова уведомления определяется приложением.
...
Возвращает указатель на выполняющийся в данный момент поток
...
Получение сведений о производительности потока
...
Изменение порога вытеснения для потока приложения
...
Эта служба возобновляет или готовит к выполнению поток, который ранее был приостановлен вызовом ***tx_thread_suspend***. Кроме того, эта служба возобновляет потоки, созданные без автоматического запуска.
...
Эта служба приводит к приостановке вызывающего потока на указанное число тактов таймера. Количество физического времени, связанное с тактом таймера, зависит от приложения. Эту службу можно вызывать только из потока приложения.
...
Эта служба завершает указанный поток приложения независимо от того, приостановлен ли поток. Поток может вызывать эту службу для завершения самого себя.
...
Изменяет временной срез потока приложения
...
Эта служба возвращает содержимое внутренних системных часов. Каждый такт таймера увеличивает значение внутренних системных часов на единицу. Во время инициализации системные часы устанавливаются в нулевое значение, а их значение можно изменить с помощью службы ***tx_time_set***.
...
Эта служба изменяет характеристики срока действия указанного таймера приложения. Перед вызовом этой службы таймер должен быть деактивирован.
...
Эта служба создает таймер приложения с указанной функцией истечения срока действия и периодичностью.
...
- **auto_activate** — определяет, активируется ли таймер автоматически во время создания. Если это значение равно **TX_AUTO_ACTIVATE** (0x01), таймер становится активным. В противном случае, если указано значение **TX_NO_ACTIVATE** (0x00), таймер создается в неактивном состоянии. В этом случае для фактического запуска таймера необходим последующий вызов службы **_tx_timer_activate_**.
...
Эта служба деактивирует указанный таймер приложения. Если таймер уже деактивирован, эта служба не оказывает никакого эффекта.
