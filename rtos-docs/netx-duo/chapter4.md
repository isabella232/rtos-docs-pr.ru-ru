...
Эта служба использует все физические интерфейсы для передачи необязательных запросов ARP, если IP-адрес интерфейса является допустимым. При последующем получении ответа ARP вызывается обработчик ответов для обработки ответа на необязательный ARP.
...
- **ip_total_bytes_received** — указатель на место назначения для общего числа полученных по протоколу IP байтов.
- **ip_invalid_packets** — указатель на место назначения для общего числа недопустимых IP-пакетов.
- **ip_receive_packets_dropped** — указатель на место назначения для общего числа полученных и необработанных пакетов.
- **ip_receive_checksum_errors** — указатель на место назначения для общего числа ошибок контрольной суммы в полученных пакетах.
- **ip_send_packets_dropped** — указатель на место назначения для общего числа отправленных и необработанных пакетов.
...
- **physical_address_msw** — указатель на место назначения для старших 16 бит MAC-адреса устройства.
- **physical_address_lsw** — указатель на место назначения для младших 32 бит MAC-адреса устройства.
...
- **NX_SUCCESS** (0x00) — успешная проверка состояния IP.
- **NX_NOT_SUCCESSFUL** (0x43) — запрос состояния не был выполнен в течение указанного времени ожидания.
- **NX_PTR_ERROR** (0x07) — указатель IP является или стал недопустимым, либо недопустим указатель фактического состояния.
- **NX_OPTION_ERROR** (0x0a) — недопустимый параметр требуемого состояния.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_INVALID_INTERFACE** (0x4C) — interface_index за пределами допустимого диапазона либо недопустимый интерфейс.
...
- **NX_SUCCESS** (0x00) — успешно задано.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель на блок управления IP или указатель на новый физический адрес.
- **NX_CALLER_ERROR** (0x11) — служба не вызывается из инициализации системы или контекста потока.
...
- **ip_ptr** — указатель на ранее созданный экземпляр IP.
- **packet_ptr** — указатель на отправляемый необработанный IP-пакет.
- **destination_ip** — IP-адрес назначения, которым может быть IP-адрес конкретного узла, сетевой широковещательный адрес, петлевой адрес или адрес многоадресной рассылки.
...
- **NX_OVERFLOW** (0x03) — недопустимый указатель начала пакета.
- **NX_UNDERFLOW** (0x02) — недопустимый указатель начала пакета.
...
Эта служба проверяет и при необходимости ожидает указанное состояние основного сетевого интерфейса ранее созданного экземпляра IP. Для получения состояния дополнительных интерфейсов в приложении должна использоваться служба ***nx_ip_interface_status_check***.
...
Эта служба выделяет пакет из указанного пула и корректирует положение указателя начала в пакете в соответствии с указанным типом пакета. Если пакет недоступен, служба приостанавливает работу в соответствии с указанным параметром ожидания.
...
- **NX_PTR_ERROR** (0x07) — недопустимый указатель на возвращаемый пул или пакет.
...
Эта служба копирует сведения из предоставленного пакета в один или несколько новых пакетов, выделенных из указанного пула пакетов. В случае успеха возвращается указатель на новый пакет в месте назначения, на которое указывает **new_packet_ptr**.

### <a name="parameters"></a>Параметры

- **packet_ptr** — указатель на исходный пакет.
- **new_packet_ptr** — указатель на место назначения, в котором нужно вернуть указатель на новую копию пакета.
- **pool_ptr** — указатель на ранее созданный пул пакетов, который используется для выделения одного или нескольких копируемых пакетов.
- **wait_option** — определяет ожидание службы в случае, если нет доступных пакетов. Параметры ожидания определяются следующим образом:
    - **NX_NO_WAIT** (0x00000000)
    - **NX_WAIT_FOREVER** (0xFFFFFFFF)
    - **значение времени ожидания в тактах** (от 0x00000001 до 0xFFFFFFFE)
...
- **NX_INVALID_PACKET** (0x12) — исходный пакет пуст или не удалось выполнить копирование.
- **NX_WAIT_ABORTED** (0x1A) — запрошенная приостановка прервана вызовом tx_thread_wait_abort.
- **NX_INVALID_PARAMETERS** (0x4D) — размер пакета не поддерживается протоколом.
...
Эта служба добавляет данные в конец указанного пакета. Указанная область данных копируется в пакет. Если недостаточно памяти и включена функция сцепления пакетов, то для выполнения запроса будет выделен один или несколько пакетов. Если функция сцепления пакетов не включена, возвращается *NX_SIZE_ERROR*.
...
- **NX_WAIT_ABORTED** (0x1A) — запрошенная приостановка прервана вызовом tx_thread_wait_abort.
- **NX_INVALID_PARAMETERS** (0x4D) — размер пакета не поддерживается протоколом.
- **NX_UNDERFLOW** (0x02) — указатель начала меньше, чем начало полезных данных.
- **NX_OVERFLOW** (0x03) — указатель для добавления больше, чем конец полезных данных.
...
Инициализация, потоки, таймеры и подпрограммы ISR (сетевые драйверы приложений)
...
Эта служба копирует данные из пакета NetX Duo (или из цепочки пакетов) начиная с указанного смещения от указателя начала пакета указанного размера в байтах в заданный буфер. Число фактически скопированных байтов возвращается в *bytes_copied*. Эта служба не удаляет данные из пакета и не настраивает указатель начала или другие сведения о внутреннем состоянии.

### <a name="parameters"></a>Параметры

- **packet_ptr** — указатель на извлекаемый пакет.
- **offset** — смещение от текущего указателя начала.
...
- **free_packets** — указатель на место назначения для общего числа пакетов, свободных в данный момент.
...
Инициализация, потоки, таймеры, сетевые драйверы приложений (в том числе подпрограммы ISR)
...
- **NX_SUCCESS** (0x00) — успешное отключение протокола RARP.
- **NX_NOT_ENABLED** (0x14) — протокол RARP не включен.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель на IP-адрес.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
...
- **NX_SUCCESS** (0x00) — успешное включение протокола RARP.
- **NX_IP_ADDRESS_ERROR** (0x21) — IP-адрес уже допустимый.
...
Получение сведений о действиях протокола RARP
...
Инициализация, потоки, таймеры, подпрограммы ISR
...
Привязка клиентского TCP-сокета к TCP-порту
...
Эта служба привязывает ранее созданный клиентский TCP-сокет к указанному TCP-порту. Допустимый диапазон TCP-сокетов — от 0 до 0xFFFF. Если указанный TCP-порт недоступен, служба приостанавливает выполнение в соответствии с указанным параметром ожидания.
...
Подключение клиентского TCP-сокета
...
Получение номера порта, привязанного к клиентскому TCP-сокету
...
Отмена привязки клиентского TCP-сокета к TCP-порту
...
Эта служба отменяет привязку между клиентским TCP-сокетом и TCP-портом. Если есть другие потоки, ожидающие привязки другого сокета к тому же номеру порта, к порту привязывается первый приостановленный поток.
...
- **NX_SUCCESS** (0x00) — успешное включение протокола TCP.
...
- **NX_SUCCESS** (0x00) — свободный порт успешно найден.
- **NX_NO_FREE_PORTS** (0x45) — нет свободных портов.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель на IP-адрес.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_NOT_ENABLED** (0x14) — этот компонент не включен.
...
Получение сведений о действиях протокола TCP
...
- **tcp_bytes_sent** — указатель на место назначения для общего числа отправленных по протоколу TCP байтов.
- **tcp_packets_received** — указатель на место назначения для общего числа полученных TCP-пакетов.
- **tcp_bytes_received** —указатель на место назначения для общего числа полученных по протоколу TCP байтов.
- **tcp_invalid_packets** — указатель на место назначения для общего числа недопустимых TCP-пакетов.
- **tcp_receive_packets_dropped** — указатель на место назначения для общего числа полученных и необработанных TCP-пакетов.
...
- **tcp_connections_dropped** — указатель на место назначения для общего числа необработанных TCP-подключений.
- **tcp_retransmit_packets** — указатель на место назначения для общего числа повторно отправленных TCP-пакетов.
...
Если приложение планирует обрабатывать дополнительные клиентские подключения через тот же порт, служба ***nx_tcp_server_socket_relisten*** должна быть вызвана с доступным сокетом (в закрытом состоянии) для следующего подключения. Дополнительные клиентские подключения помещаются в очередь, пока не будет вызвана служба повторного ожидания передачи данных. При превышении максимальной глубины очереди самый старый запрос на подключение отменяется в пользу нового. Максимальная глубина очереди задается данной службой.
...
- **NX_DUPLICATE_LISTEN** (0x34) — для этого порта уже есть активный запрос на ожидание передачи данных.
- **NX_INVALID_PORT** (0x46) — указан недопустимый порт.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель на IP-адрес или сокет.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_NOT_ENABLED** (0x14) — этот компонент не включен.
...
- **NX_NOT_CLOSED** (0x35) — указанный сокет сервера не находится в закрытом состоянии.
- **NX_ALREADY_BOUND** (0x22) — указанный сокет сервера уже привязан к порту.
- **NX_INVALID_RELISTEN** (0x47) — для этого порта уже есть допустимый указатель сокета или у указанного порта нет активного запроса на ожидание передачи данных.
...
Получает число байтов, доступных для получения
...
Эта служба получает число байтов, доступных для получения в указанном TCP-сокете. Обратите внимание, что TCP-сокет уже должен быть подключен.

### <a name="parameters"></a>Параметры

- **socket_ptr** — указатель на ранее созданный и подключенный TCP-сокет.
...
Создание сокета сервера или клиента TCP
...
- **ip_ptr** — указатель на ранее созданный экземпляр IP.
- **socket_ptr** — указатель на новый блок управления TCP-сокета.
- **name** — имя приложения для этого TCP-сокета.
- **type_of_service** — определяет тип службы для передачи. Допустимые значения:
    - **NX_IP_NORMAL** (0x00000000)
    - **NX_IP_MIN_DELAY** (0x00100000)
    - **NX_IP_MAX_DATA** (0x00080000)
...
- **time_to_live** — задает 8-битное значение, определяющее, сколько маршрутизаторов этот пакет может пройти перед тем, как будет отброшен. Значение по умолчанию задается в параметре NX_IP_TIME_TO_LIVE.
...
- **disconnect_callback** — функция приложения, которая вызывается каждый раз, когда сокет выдает команду на завершение подключения на другом конце подключения. Если это значение равно NX_NULL, функция обратного вызова для завершения подключения отключена.

### <a name="return-values"></a>Возвращаемые значения 

- **NX_SUCCESS** (0x00) — сокет клиента TCP успешно создан.
...
Эта служба удаляет созданный ранее TCP-сокет. Если сокет по-прежнему привязан или подключен, служба возвращает код ошибки.

### <a name="parameters"></a>Параметры

- **socket_ptr** — ранее созданный TCP-сокет.
...
- **NX_PTR_ERROR** (0x07) — недопустимый указатель сокета.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_NOT_ENABLED** (0x14) — протокол TCP не включен приложением.
...
Получение сведений о действиях TCP-сокета
...
- **tcp_bytes_sent** — указатель на место назначения для общего числа отправленных через сокет байтов TCP.
- **tcp_packets_received** — указатель на место назначения для общего числа полученных через сокет TCP-пакетов.
- **tcp_bytes_received** — указатель на место назначения для общего числа полученных через сокет байтов TCP.
- **tcp_retransmit_packets** — указатель на место назначения для общего числа повторно отправленных TCP-пакетов.
...
- **NX_SUCCESS** (0x00) — успешное получение сведений о TCP-сокете.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель сокета. 
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_NOT_ENABLED** (0x14) — этот компонент не включен.
...
Получение MSS однорангового TCP-сокета
...
Эту службу следует использовать до того, как TCP-сокет начнет процесс подключения. Если служба используется после установления TCP-подключения, новое значение для него не действует.
...
- **NX_SUCCESS** (0x00) — успешное указание MSS.
...
Получение сведений об одноранговом TCP-сокете
...
- **peer_ip_address** — указатель на место назначения для однорангового IP-адреса в порядке байтов узла.
- **peer_port** — указатель на место назначения для номера однорангового порта в порядке байтов узла.

### <a name="return-values"></a>Возвращаемые значения  

- **NX_SUCCESS** (0x00) — служба успешно выполняется. Одноранговый IP-адрес и номер порта возвращаются вызывающему объекту.
- **NX_NOT_CONNECTED** (0x38) — сокет не находится в подключенном состоянии.
- **NX_PTR_ERROR** (0x07) — недопустимые указатели.
- **NX_NOT_ENABLED** (0x14) — протокол TCP не включен.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
...
Получение данных из TCP-сокета
...
Эта служба настраивает указатель на функцию уведомления о получении с помощью функции обратного вызова, заданной приложением. Эта функция обратного вызова затем вызывается при получении одного или нескольких пакетов в сокете. Если задан указатель NX_NULL, функция уведомления отключена.

### <a name="parameters"></a>Параметры

- **socket_ptr** — указатель на TCP-сокет.
- **tcp_receive_notify** — указатель на функцию обратного вызова приложения, вызываемую при получении одного или нескольких пакетов в сокете.

### <a name="return-values"></a>Возвращаемые значения 

- **NX_SUCCESS** (0x00) — уведомление об успешном получении сокета.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель сокета.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_NOT_ENABLED** (0x14) — функция TCP не включена.
...
Эта служба отправляет данные TCP через подключенный ранее TCP-сокет. Если последний объявленный размер окна получателя меньше, чем этот запрос, служба при необходимости приостанавливает выполнение в соответствии с указанным параметром ожидания. Эта служба гарантирует, что на уровень IP будут отправлены пакеты данных объемом, не превышающим MSS. 
...
- **socket_ptr** — указатель на ранее подключенный экземпляр TCP-сокета.
...
- **NX_NOT_CONNECTED** (0x38) — сокет больше не подключен.
- **NX_WINDOW_OVERFLOW** (0x39) — запрос превышает объявленный размер окна получателя в байтах.
- **NX_WAIT_ABORTED** (0x1A) — запрошенная приостановка прервана вызовом tx_thread_wait_abort.
...
Ожидание перехода TCP-сокета в определенное состояние
...
- **socket_ptr** — указатель на ранее подключенный экземпляр сокета клиента или сервера.
- **tcp_timed_wait_callback** — функция обратного вызова с состоянием ожидания с привязкой ко времени.
...
Эта служба настраивает различные параметры передачи для указанного TCP-сокета.
...
- **timeout_shift** — значение, на которое сдвигается время ожидания для каждой последующей попытки. Если задать значение 0, время ожидания между последовательными повторными попытками будет одинаковым. Если задать значение 1, время ожидания между попытками будет удвоено.
...
Получение сведений о действиях протокола UDP
...
- **udp_bytes_sent** — указатель на место назначения для общего числа отправленных по протоколу UDP байтов.
- **udp_packets_received** — указатель на место назначения для общего числа полученных UDP-пакетов.
- **udp_bytes_received** — указатель на место назначения для общего числа полученных по протоколу UDP байтов.
- **udp_invalid_packets** — указатель на место назначения для общего числа недопустимых UDP-пакетов.
- **udp_receive_packets_dropped** — указатель на место назначения для общего числа полученных и необработанных UDP-пакетов.
...
Привязка UDP-сокета к UDP-порту
...
Эта служба привязывает ранее созданный UDP-сокет к указанному UDP-порту. Допустимый диапазон UDP-сокетов — от 0 до 0xFFFF. Если запрошенный номер порта привязан к другому сокету, эта служба в течение указанного времени ожидает отмены привязки сокета к номеру порта.
...
Эта служба получает число байтов, доступных для приема в указанном UDP-сокете.

### <a name="parameters"></a>Параметры

- **socket_ptr** — указатель на ранее созданный UDP-сокет.
- **bytes_available** — указатель на место назначения для доступных байтов.
...
Отключение контрольной суммы для UDP-сокета
...
Включение контрольной суммы для UDP-сокета
...
Эта служба включает логику проверки контрольной суммы при отправке и получении пакетов через указанный UDP-сокет. Контрольная сумма охватывает всю область данных UDP, а также псевдозаголовок IP.
...
Эта служба создает UDP-сокет для указанного экземпляра IP.

### <a name="parameters"></a>Параметры

- **ip_ptr** — указатель на ранее созданный экземпляр IP.
- **socket_ptr** — указатель на новый блок управления UDP-сокетом.
- **name** — имя приложения для этого UDP-сокета.
- **type_of_service** — определяет тип службы для передачи. Допустимые значения:
    - **NX_IP_NORMAL** (0x00000000)
    - **NX_IP_MIN_DELAY** (0x00100000)
    - **NX_IP_MAX_DATA** (0x00080000)
...
- **NX_SUCCESS** (0x00) — UDP-сокет успешно создан.
...
Эта служба удаляет созданный ранее UDP-сокет. Если сокет привязан к порту, сначала необходимо отменить привязку.
...
Получение сведений о действиях UDP-сокета
...
- **udp_receive_packets_dropped** — указатель на место назначения для общего числа UDP-пакетов, полученных и необработанных для сокета из-за превышения размера очереди.
- **udp_checksum_errors** — указатель на место назначения для общего числа UDP-пакетов с ошибками контрольной суммы в сокете.

### <a name="return-values"></a>Возвращаемые значения 

- **NX_SUCCESS** (0x00) — успешное получение сведений о UDP-сокете.
- **NX_PTR_ERROR** (0x07) — недопустимый указатель сокета.
- **NX_CALLER_ERROR** (0x11) — недопустимый вызывающий объект этой службы.
- **NX_NOT_ENABLED** (0x14) — этот компонент не включен.
...
Выбор номера порта, привязанного к UDP-сокету
...
Получение датаграммы из UDP-сокета
...
- **socket_ptr** — указатель на UDP-сокет.
...
- **NX_INVALID_PORT** (0x46) — номер порта находится за пределами допустимого диапазона.
...
Эта служба отправляет датаграмму UDP через ранее созданный и привязанный UDP-сокет и через сетевой интерфейс с IP-адресом, указанным в качестве адреса источника. Обратите внимание, что эта служба возвращает управление немедленно независимо от того, была ли датаграмма UDP успешно отправлена. ***nxd_udp_socket_source_send*** работает как в сети IPv4, так и в сети IPv6.

### <a name="parameters"></a>Параметры 

- **socket_ptr** — сокет для передачи пакета.
- **packet_ptr** — указатель на передаваемый пакет.
- **ip_address** — IP-адрес назначения, на который нужно отправить пакет.
...
Эта служба отменяет привязку между UDP-сокетом и UDP-портом. Все полученные пакеты, хранящиеся в очереди приема, освобождаются в рамках операции отмены привязки.
...
- **packet_ptr** — указатель на пакет датаграммы UDP.
- **ip_address** — допустимый указатель на переменную возвращаемого IP-адреса.
- **port** — допустимый указатель на переменную возвращаемого порта.

### <a name="return-values"></a>Возвращаемые значения 

- **NX_SUCCESS** (0x00) — успешное извлечение IP-адреса или порта источника.
