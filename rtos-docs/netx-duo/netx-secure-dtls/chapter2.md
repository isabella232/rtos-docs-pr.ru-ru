...
ms.locfileid: "116784093"
---
# <a name="chapter-2-installation-and-use-of-azure-rtos-netx-secure-dtls"></a>Глава 2. Установка и использование ОСРВ Azure NetX Secure DTLS

В этой главе описываются различные проблемы, связанные с установкой, настройкой и использованием компонента ОСРВ Azure NetX Secure DTLS.

## <a name="product-distribution"></a>Распространение продукта

Пакет NetX Secure доступен по адресу [https://github.com/azure-rtos/netx](https://github.com/azure-rtos/netx). Он содержит файлы исходного кода, дополнительные файлы и PDF-файл, содержащий этот документ:

- **nx_secure_dtls_api.h**: файл заголовка общедоступного API для NetX Secure DTLS;
- **nx_secure_dtls_user.h**: здесь пользователь может определить файл заголовка для NetX Secure DTLS;
- **nx_secure_ port.h** — соответствующие различным платформам определения для NetX Secure;
- **nx_secure_dtls.h**: файл заголовка для NetX Secure DTLS;
- **nx_secure_tls.h**: файл заголовка для NetX Secure TLS;
- **nx_secure_dtls\*.c/h**: файлы исходного кода C/H для NetX Secure DTLS;
...
- **demo_netx_secure_dtls.c**: демонстрационный файл исходного кода C для NetX Secure DTLS;
- **NetX_Secure_DTLS_User_Guide.pdf**: PDF-файл с описанием продукта NetX Secure.

> [!NOTE]
> Файлы nx_crypto* предоставляются для различных аппаратных платформ в подкаталоге родительского каталога NetX Secure.

## <a name="netx-secure-dtls-installation"></a>Установка NetX Secure DTLS

Чтобы использовать NetX Secure DTLS, весь дистрибутив, упомянутый ранее, должен быть скопирован на уровень каталога, где установлен NetX. Например, если пакет NetX установлен в каталог *\threadx\arm7\NetX*, то каталоги *nx_secure\*\*.* следует скопировать в *\threadx\arm7\NetXSecure*.

## <a name="using-netx-secure-dlts"></a>Использование NetX Secure DLTS

Использовать NetX Secure DTLS просто. В код приложения следует добавить *nx_secure_dtls_api.h* после *tx_api.h* и *nx_api.h* (для ThreadX и NetX соответственно). После добавления *nx_secure_dtls_api.h* код приложения сможет выполнять вызовы функций NetX Secure DTLS, указанные далее в этом руководстве. Приложение также должно импортировать файлы *nx_secure \*.\** в библиотеку NetXSecure, а соответствующие платформе файлы *nx_crypto\*.\** — в библиотеку NetXCrypto, которая затем связывается с окончательным двоичным файлом приложения.

## <a name="small-example-system-dtls-client"></a>Небольшой пример системы (клиент DTLS)

Пример того, как просто использовать NetX Secure DTLS, показан на рисунке 1.1 ниже. На нем отображен простой клиент DTLS, предназначенный для работы с сервером DTLS с OpenSSL (или аналогичным). Обратите внимание на то, что структура клиентской программы DTLS очень похожа на клиент NetX Secure TLS (см. документацию по NetX Secure TLS). Это обусловлено тем, что протокол DTLS, по сути, является версией TLS, предназначенной для менее надежных транспортных сетевых протоколов, таких как UDP.
...
## <a name="small-example-system-dtls-server"></a>Небольшой пример системы (сервер DTLS)

Пример того, как просто использовать NetX Secure, показан на рисунке 1.2 ниже, где отображен простой сервер DTLS. Обратите внимание на то, что функциональные возможности сервера DTLS существенно отличаются от клиента DTLS, а также клиента или сервера TLS, так как сервер DTLS должен управлять множеством входящих запросов клиентов в одном UDP-порте (хранимом в экземпляре сервера DTLS).
...
**Рисунок 1.2. Пример сервера NetX Secure DTLS**

## <a name="configuration-options"></a>Параметры конфигурации

Существует ряд параметров конфигурации для создания NetX Secure.
Ниже приведен список всех параметров, каждый из которых подробно описан ниже.

| Определение                                                 | Значение                                                                                                                                                                                                                |
|--------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **NX_SECURE_ENABLE_DTLS**                           | Этот макрос должен быть определен для обеспечения логики DTLS в NetX Secure.                                                                                                                                                       |
...
| **NX_SECURE_TLS_CLIENT_DISABLED**                  | Этот параметр позволяет удалить весь код стека TLS или DTLS, связанный с режимом клиента, уменьшая объем кода и использование данных.                                                                                                            |
| **NX_SECURE_TLS_SERVER_DISABLED**                  | Этот параметр позволяет удалить весь код стека TLS или DTLS, связанный с режимом сервера, уменьшая объем кода и использование данных.                                                                                                            |
