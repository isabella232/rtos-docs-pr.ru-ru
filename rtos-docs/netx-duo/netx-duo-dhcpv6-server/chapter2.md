...
ms.locfileid: "104814744"
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a><span data-ttu-id="fb54f-103">Глава 2. Установка и использование DHCPv6-сервера NetX Duo для ОСРВ Azure</span><span class="sxs-lookup"><span data-stu-id="fb54f-103">Chapter 2 - Installation and use of Azure RTOS NetX Duo DHCPv6 server</span></span>

<span data-ttu-id="fb54f-104">В этой главе описываются различные проблемы, связанные с установкой, настройкой и использованием DHCPv6-сервера NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="fb54f-104">This chapter contains a description of various issues related to installation, setup, and usage of the NetX Duo DHCPv6 Server.</span></span>

## <a name="product-distribution"></a><span data-ttu-id="fb54f-105">Распространение продукта</span><span class="sxs-lookup"><span data-stu-id="fb54f-105">Product Distribution</span></span>

<span data-ttu-id="fb54f-106">NetX Duo для ОСРВ Azure можно получить из нашего общедоступного репозитория исходного кода по адресу [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/).</span><span class="sxs-lookup"><span data-stu-id="fb54f-106">Azure RTOS NetX Duo can be obtained from our public source code repository at [https://github.com/azure-rtos/netxduo/](https://github.com/azure-rtos/netxduo/).</span></span>

<span data-ttu-id="fb54f-107">**nxd_dhcpv6_server.h** — файл заголовка DHCPv6-сервера NetX Duo</span><span class="sxs-lookup"><span data-stu-id="fb54f-107">**nxd_dhcpv6_server.h** NetX DuoDHCPv6Server header file</span></span>

<span data-ttu-id="fb54f-108">**nxd_dhcpv6_server.c** — исходный файл DHCPv6-сервера NetX Duo</span><span class="sxs-lookup"><span data-stu-id="fb54f-108">**nxd_dhcpv6_server.c** NetX DuoDHCPv6Server source file</span></span>

<span data-ttu-id="fb54f-109">**demo_netxduo_dhcpv6.c** — демонстрационный файл DHCPv6-сервера NetX Duo</span><span class="sxs-lookup"><span data-stu-id="fb54f-109">**demo_netxduo_dhcpv6.c** NetX Duo DHCPv6 Server demo file</span></span>

<span data-ttu-id="fb54f-110">**nxd_dhcpv6_server.pdf** — руководство пользователя DHCPv6-сервера NetX Duo</span><span class="sxs-lookup"><span data-stu-id="fb54f-110">**nxd_dhcpv6_server.pdf** NetX Duo DHCPv6Server User Guide</span></span>

## <a name="netx-duo-dhcpv6-server-installation"></a><span data-ttu-id="fb54f-111">Установка DHCPv6-сервера NetX Duo</span><span class="sxs-lookup"><span data-stu-id="fb54f-111">NetX Duo DHCPv6 Server Installation</span></span>

<span data-ttu-id="fb54f-112">Чтобы использовать интерфейс API DHCPv6-сервера для NetX Duo, весь дистрибутив, упомянутый ранее, нужно скопировать в каталог, где установлено решение NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="fb54f-112">In order to use the NetX Duo DHCPv6Server API, the entire distribution mentioned previously should be copied to the same directory where NetX Duo is installed.</span></span> <span data-ttu-id="fb54f-113">Например, если NetX Duo установлен в каталоге *\threadx\arm7\green*, файлы *nxd_dhcpv6_server.h* и *nx_dhpcv6_server.c* следует скопировать в него.</span><span class="sxs-lookup"><span data-stu-id="fb54f-113">For example, if NetX Duo is installed in the directory “*\threadx\arm7\green*” then the *nxd_dhcpv6_server.h* and *nx_dhpcv6_server.c* files should be copied into this directory.</span></span>

## <a name="using-netx-duo-dhcpv6-server"></a><span data-ttu-id="fb54f-114">Использование DHCPv6-сервера NetX Duo</span><span class="sxs-lookup"><span data-stu-id="fb54f-114">Using NetX Duo DHCPv6 Server</span></span>

<span data-ttu-id="fb54f-115">Использовать интерфейс API DHCPv6-сервера NetX Duo очень просто.</span><span class="sxs-lookup"><span data-stu-id="fb54f-115">Using the NetX Duo DHCPv6Server API is easy.</span></span> <span data-ttu-id="fb54f-116">В код приложения достаточно добавить файл *nx_dhcpv6-server.h* после *tx_api.h* и *nx_api.h*, чтобы использовать ThreadX и NetX Duo соответственно.</span><span class="sxs-lookup"><span data-stu-id="fb54f-116">Basically, the application code must include *nx_dhcpv6-server.h* after it includes *tx_api.h* and *nx_api.h*, in order to use ThreadX and NetX Duo, respectively.</span></span> <span data-ttu-id="fb54f-117">В приложение также следует включить файл *nxd_dhcpv6_server.c* в процессе сборки.</span><span class="sxs-lookup"><span data-stu-id="fb54f-117">The application must also include *nxd_dhcpv6_server.c* in the build process.</span></span> <span data-ttu-id="fb54f-118">Этот файл должен быть скомпилирован так же, как и другие файлы приложения, а его объектная форма должна быть связана с файлами приложения.</span><span class="sxs-lookup"><span data-stu-id="fb54f-118">This file must be compiled in the same manner as other application files and its object form must be linked along with the files of the application.</span></span> <span data-ttu-id="fb54f-119">Это все, что необходимо для использования DHCPv6-сервера в NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="fb54f-119">This is all that is required to use NetX Duo DHCPv6 Server.</span></span>

<span data-ttu-id="fb54f-120">Обратите внимание, что, поскольку протокол DHCPv6 основан на протоколе IPv6, протокол IPv6 должен быть включен в экземпляре IP с помощью *nxd_ipv6_enable*.</span><span class="sxs-lookup"><span data-stu-id="fb54f-120">Note that since DHCPv6is based on the IPv6 protocol, IPv6 must be enabled on the IP instance using *nxd_ipv6_enable*.</span></span> <span data-ttu-id="fb54f-121">Также используются службы UDP и ICMPv6 NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="fb54f-121">NetX Duo UDP and ICMPv6 services are also utilized.</span></span> <span data-ttu-id="fb54f-122">Протокол UDP включается путем вызова *nx_udp_enable*, а протокол ICMPv6 — путем вызова *nxd_icmp_enable* перед запуском задачи потока DHCPv6-сервера NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="fb54f-122">UDP is enabled by calling *nx_udp_enable* and ICMPv6is enabled by calling *nxd_icmp_enable* prior to starting the NetX Duo DHCPv6 Server thread task.</span></span>

## <a name="small-example-system"></a><span data-ttu-id="fb54f-123">Пример небольшой системы</span><span class="sxs-lookup"><span data-stu-id="fb54f-123">Small Example System</span></span>

<span data-ttu-id="fb54f-124">Приведенный ниже пример демонстрирует, как просто пользоваться DHCPv6-сервером NetX Duo. В нем применяется клиент DHCPv6 и сервер, работающий на основе виртуального драйвера ОЗУ.</span><span class="sxs-lookup"><span data-stu-id="fb54f-124">An example of how easy it is to use the NetX Duo DHCPv6 Server is described in the small example below using a DHCPv6 Client and Server running over a virtual “RAM” driver.</span></span> <span data-ttu-id="fb54f-125">В этой демонстрации предполагается единственный узел со средой NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="fb54f-125">This demo assumes a single homed host using the NetX Duo environment.</span></span>

<span data-ttu-id="fb54f-126">*tx_application_define* создает пул пакетов для отправки сообщения DHCPv6, поток и экземпляр IP как для клиента, так и для сервера, а также включает протоколы UDP (DHCP работает через UDP), IPv6, ICMP и ICMPv6 для задач IP клиента и сервера в строках 116–157.</span><span class="sxs-lookup"><span data-stu-id="fb54f-126">*tx_application_define* creates packet pool for sending DHCPv6 message, a thread and an IP instance for both the Client and Server, and enables UDP (DHCP runs over UDP), IPv6, ICMP and ICMPv6 for both Client and Server IP tasks in lines 116-157.</span></span>

<span data-ttu-id="fb54f-127">DHCPv6-сервер создается в строке 456.</span><span class="sxs-lookup"><span data-stu-id="fb54f-127">The DHCPv6 Server is created in line 456.</span></span> <span data-ttu-id="fb54f-128">Дополнительные обработчики отклонения адреса или запроса параметров не определяются.</span><span class="sxs-lookup"><span data-stu-id="fb54f-128">It does not define the optional address decline or option request handlers.</span></span> <span data-ttu-id="fb54f-129">Во входной функции потока сервера настраивается IP-адрес сервера со службами связывания локальных адресов в строках 435–453.</span><span class="sxs-lookup"><span data-stu-id="fb54f-129">In the Server thread entry function, the Server IP is set up with a link local address services in lines 435-453.</span></span>

<span data-ttu-id="fb54f-130">Перед запуском DHCPv6-сервера ведущее приложение создает идентификатор DUID сервера в строке 498 и задает DNS-сервер локальной сети в строке 483.</span><span class="sxs-lookup"><span data-stu-id="fb54f-130">Before starting the DHCPv6 Server, the host application creates a Server DUID in line 498 and sets the local network DNS server on line 483.</span></span> <span data-ttu-id="fb54f-131">Затем создается таблица назначаемых IP-адресов в строке 521.</span><span class="sxs-lookup"><span data-stu-id="fb54f-131">It then creates a table of assignable IP addresses in lines 521.</span></span> <span data-ttu-id="fb54f-132">Сведения о хранении и извлечении таблиц сервера из памяти см. в разделе **Расширенный пример системы** в приложении D.</span><span class="sxs-lookup"><span data-stu-id="fb54f-132">See the **Advanced Example System** in Appendix D for how to store and retrieve Server tables from memory.</span></span>

<span data-ttu-id="fb54f-133">После этого DHCPv6-сервер подготавливается к запуску в строке 530.</span><span class="sxs-lookup"><span data-stu-id="fb54f-133">Then the DHCPv6 Server is ready to start on line 530.</span></span>

<span data-ttu-id="fb54f-134">Подробные сведения о создании и запуске клиента DHCPv6 NetX Duo см. в файле *nxd_dhcpv6_client.pdf*, распространяемом вместе с DHCPv6-сервером.</span><span class="sxs-lookup"><span data-stu-id="fb54f-134">For details on creating and running the NetX Duo DHCPv6 Client see the *nxd_dhcpv6_client.pdf* file distributed on with the DHCPv6 Server.</span></span>
...
<span data-ttu-id="fb54f-135">**Рис. 6. Пример DHCPv6-сервера NetX Duo**</span><span class="sxs-lookup"><span data-stu-id="fb54f-135">**Figure 6. Example of the NetX Duo DHCPv6 Server**</span></span>