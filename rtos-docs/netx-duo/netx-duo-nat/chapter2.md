...
---
# <a name="chapter-2---installation-and-use-of-nat"></a>Глава 2. Установка и использование NAT

В этой главе содержится описание способов установки, настройки и использования служб NAT для NetX Duo.

## <a name="netx-duo-nat-installation"></a>Установка NAT для NetX Duo

NAT для NetX Duo можно получить по адресу [https://github.com/azure-rtos/netxduo](https://github.com/azure-rtos/netxduo). Пакет NAT для NetX Duo включает один исходный файл и один файл заголовка, а также файл демонстрационного приложения и PDF-файл с этим документом, как представлено ниже.

- **nx_nat. c** — исходный файл C для NAT для NetX Duo;
- **nx_nat.h** — файл заголовка C для NAT для NetX Duo;
- **demo_netx_nat. c** — пример файла исходного кода C для узла NetX Duo;
- **nx_nat.docx** — руководство пользователя NAT для NetX Duo (этот документ).

Скопируйте файлы исходного кода NAT для NetX Duo в тот же каталог, где установлены NetX Duo и ThreadX. Например, если NetX Duo и ThreadX установлены в каталоге *\threadx\mcf5485\green*, то именно в него нужно скопировать файлы *nx_nat.c*, *nx_nat.h и измененные файлы NetX Duo*. Скопируйте измененные файлы NetX Duo поверх существующих файлов NetX Duo. Скопируйте также в этот каталог файлы драйверов для контроллера Ethernet.

Чтобы скомпилировать приложение NAT для NetX Duo, выполните следующие действия.

- При компиляции библиотеки NetX Duo *nxduo.a* должен быть определен параметр NX_NAT_ENABLED. Это можно сделать в файле *nx_user.h*(определите также NX_INCLUDE_USER_DEFINE_FILE, чтобы параметры конфигурации из *nx_user.h* включались в сборку).
- Проект приложения должен включать файл *nx_nat.h* после *tx_api.h* и *nx_api.h*. Эти два файла заголовка необходимы для использования служб ThreadX и NetX Duo.
- Затем приложение включает NAT на ранее созданном экземпляре IP с помощью службы *nx_nat_enable*.
- Код приложения может включать или отключать NAT по мере необходимости, вызывая службы *nx_nat_enable* и *nx_nat_disable*.
- Чтобы создать исполняемый файл, код проекта приложения компилируется и связывается с библиотекой NetX Duo с поддержкой NAT.
- Чтобы обеспечить поддержку подключений NAT по протоколам TCP, UDP или ICMP, для NetX Duo необходимо включить поддержку соответствующего протокола. Для этого вызовите службу *nx_tcp_enable, nx_udp_enable* и (или) *nx_icmp_enable* для ранее созданного экземпляра IP.

## <a name="small-example-demo-nat-setup"></a>Небольшой пример для демонстрации настройки NAT

На рисунке 4 ниже функция *tx_application_define* демонстрирует настройку NAT для NetX Duo в приложении. В отличие от большинства демонстрационных файлов NetX Duo, которые распространяются на установочном компакт-диске, этот пример выполняется не на компьютере под управлением ОС Windows, а непосредственно на плате процессора с двумя контроллерами Ethernet, используя драйвер виртуальной сети *_nx_ram_network_driver*(). Устройство NAT подключается к локальному домену через локальный коммутатор на локальном интерфейсе, а к внешней сети — через второй коммутатор на внешнем интерфейсе.

Базовая конфигурация NetX Duo представлена в файле demo_netx_nat.c. Частная сеть определяется как 192.168.2.xx и содержит два локальных узла. Глобальная сеть определяется как 192.168.0.xx, а шлюз для исходящих из сети пакетов имеет адрес 192.168.0.1. В строках 118–171 создаются экземпляры IP для NetX Duo и вызывается драйвер "ram", при этом два интерфейса, подключенные к экземпляру nat_ip, выполняют роль маршрутизатора NAT, подключенный к интерфейсу экземпляр local_ip выполняет роль локального узла, а подключенный к интерфейсу экземпляр external_ip выполняет роль внешнего узла.

В строке 252 создается NAT и вызывается кэш для хранения записей динамического преобразования. В строке 319 включается функция NAT, в строке 362 создается запись статического преобразования для входящих подключений, чтобы внешний узел мог получить доступ к локальному узлу.
...
**Рис. 5. Настройка NAT для NetX Duo**
