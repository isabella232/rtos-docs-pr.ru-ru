...
В этой главе описываются различные вопросы, связанные с установкой, настройкой и использованием PTP-клиента для NetX Duo.

## <a name="product-distribution"></a>Распространение продукта

NetX Duo для ОСРВ Azure можно получить из общедоступного репозитория исходного кода по адресу https://github.com/azure-rtos/netxduo/tree/master/addons/ptp.

***nxd_ptp_client.h*** — файл заголовка для PTP-клиента для NetX Duo. ***nxd_ptp_client.c*** — исходный файл C для PTP-клиента для NetX Duo. ***demo_netx_duo_ptp_client.c*** — демонстрационный пример PTP-клиента для NetX Duo.


## <a name="using-ptp-client"></a>Использование PTP-клиента
Использование PTP-клиента для NetX Duo не представляет никаких трудностей. В коде приложения следует включить файл ***nxd_ptp_client.h** _ после включения _*_tx_api.h_*_ и _*_nx_api.h_*_, что использовать ThreadX и NetX Duo соответственно. После включения файла заголовка для PTP-клиента код приложения сможет выполнять вызовы функций PTP-клиента, которые описаны далее в этом руководстве. Приложение также должно включить файл _ *_nxd_ptp_client.c_** в ходе сборки. Этот файл должен быть скомпилирован так же, как и другие файлы приложения, а его форма объекта должна быть связана с файлами приложения. Это все, что необходимо для использования PTP-клиента для NetX Duo.

## <a name="small-example-system"></a>Пример небольшой системы
Пример использования служб PTP-клиента для NetX Duo приведен на рис. 1 ниже.
```C
/*
...
Существует несколько параметров для настройки PTP-клиента для NetX Duo. Ниже приведен список всех параметров с подробным описанием:
* **NX_PTP_CLIENT_THREAD_TIME_SLICE** определяет интервал времени для потока PTP-клиента. По умолчанию интервал времени не используется.
* **NX_PTP_CLIENT_TIMER_TICKS_PER_SECOND** определяет частоту внутреннего таймера PTP-клиента. По умолчанию используется значение 10, что означает длительность такта в 100 мс.
* **NX_PTP_CLIENT_ANNOUNCE_RECEIPT_TIMEOUT** определяет максимальное количество отсутствующих пакетов Announce, после которого завершается время ожидания. Значение по умолчанию равно 3.
* **NX_PTP_CLIENT_LOG_ANNOUNCE_INTERVAL** определяет интервал времени между двумя последовательными пакетами Announce, выраженный в формате логарифма по основанию 2. Это значение должно быть одинаковым в пределах домена. По умолчанию используется значение 1, что обозначает интервал в 2 с.
* **NX_PTP_CLIENT_DELAY_REQ_INTERVAL** определяет интервал отправки для пакетов запроса задержки (Delay). По умолчанию используется значение в 2 с.
* **NX_PTP_CLIENT_MAX_QUEUE_DEPTH** определяет максимальную глубину очереди для сокета клиента. Значение по умолчанию — 5.