...
Клиент POP3 для NetX содержит один исходный файл, один файл заголовка и демонстрационный файл. Для служб дайджеста MD5 доступно два дополнительных файла. Также есть PDF-файл с руководством пользователя (этот документ).
...
Пример использования служб клиента POP3 для NetX Duo описан ниже на рис. 1. В нем настраиваются два обратных вызова для уведомления о скачивании почты и завершении сеанса (строки 37 и 38). Пул пакетов POP3-клиента создается в строке 76. Задача потока IP создается в строке 88. Обратите внимание, что этот пул пакетов также используется для пула пакетов POP3-клиента. Протокол TCP включен для задачи IP в строке 107.
