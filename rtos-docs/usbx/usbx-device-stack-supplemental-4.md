...
Прототип этой функции определяется как:
...
Этот класс реализует универсальный слой PTP, размещенный между стеком USB и слоем Pictbridge.
...
Конечный автомат узла DPS перейдет в состояние бездействия и будет ожидать новое задание для печати.
