...
## <a name="general-considerations-for-bulk-transfer"></a>Общие рекомендации по массовой передаче данных

В соответствии со спецификацией USB 2.0 конечная точка должна всегда передавать полезные данные с помощью поля данных, объем которого меньше или равен указанному значению wMaxPacketSize для конечной точки. Размер пакета данных ограничен значением bMaxPacketSize. Передачу можно выполнить в следующих случаях:
1. Конечная точка передала ожидаемый объем данных.
2. Когда конечная точка устройства или узла получает пакет размером меньше максимального (wMaxPacketSize). Этот короткий пакет указывает на то, что пакетов данных больше не осталось и передача завершена. Если все передаваемые пакеты данных равны значению wMaxPacketSize, то окончание передачи невозможно определить. Для завершения передачи должен отправляться пакет нулевого размера (ZLP). Короткие пакеты и пакеты нулевого размера указывают на завершение массовой передачи данных. Приведенные выше рекомендации относятся к интерфейсам API массовой передачи необработанных данных, например ux_device_class_cdc_acm_read().
...
> [!Note]
> Эта функция считывает большой объем необработанных данных из узла, поэтому она ожидает, пока буфер не будет заполнен или узел не завершит передачу коротким пакетом (включая пакет нулевого размера). Дополнительные сведения см. в разделе [**Общие рекомендации по массовой передаче данных**](#general-considerations-for-bulk-transfer).
